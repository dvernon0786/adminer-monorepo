 ...
 * Processes ads in small batches to stay within rate limits
 */

const { rateLimiter } = require('./rate-limiter.js');
const { FallbackAnalyzer } = require('./fallback-analyzer.js');

class BatchProcessor {
  constructor() {
    this.batchSize = 1; // Process one ad at a time to avoid rate limits
    this.delayBetweenBatches = 20000; // 20 seconds between batches
    this.maxRetries = 3;
    this.useInngestDelays = true; // Use Inngest step.sleep for delays
  }

  /**
   * Process ads in small batches with rate limiting using Inngest delays
   */
  async processAdsInBatches(adsData, analyzer, step = null) {
  ...
  recordUsage(contentType, analysisResult) {
    ...
    let tokensUsed = 1000; // Base estimate
