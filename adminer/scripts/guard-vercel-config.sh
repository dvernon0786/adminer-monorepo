...
if ! grep -q '"buildCommand": "cd apps/api' "../vercel.json"; then
    ...
    echo "   Expected: cd apps/api && npm ci && npm run build"
    exit 1
fi
...
echo "✅ Build command uses correct path (apps/api)"
echo "✅ Output directory uses correct path (apps/api/.next)"
echo "✅ Install command uses correct path (apps/api)"
